...
def densest_subgraph_LDP(G, epsilon):
    ...
    while S and T:
        ...
        # 更新 S 和 T
        S = {v for v in G_prime.nodes() if dout_e[v] > threshold+error_tolerance }
        T = {v for v in G_prime.nodes() if din_e[v] > threshold+error_tolerance }
        # S = {v for v in G_prime.nodes() if G_prime.out_degree(v) > 0}
        # T = {v for v in G_prime.nodes() if G_prime.in_degree(v) > 0}
