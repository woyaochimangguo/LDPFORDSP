...
def get_random_subgraph(G, num_nodes=4000):
    ...
    selected_nodes = random.sample(list(G.nodes()), min(num_nodes, len(G.nodes())))
    return G.subgraph(selected_nodes).copy()

plt.rcParams['font.sans-serif'] = ['Arial']
plt.rcParams['axes.unicode_minus'] = False

output_draw_dir = "epin_1248"
...
G_full = ReadSocEpinions(file_path)  # Read the full graph
G = get_random_subgraph(G_full, num_nodes=4000)  # Extract a subgraph with 3000-5000 nodes
# 2. Baseline
